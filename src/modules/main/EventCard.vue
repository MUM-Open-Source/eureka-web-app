<template>
    <div class="event-card pad--2 mar__b--4 mar__t--2">
        <!-- Event image -->
        <SquareImage
            :imgName="event.image_url"
            alt="User"
            class="event-card__img text--center "
        />
        <!-- Like event -->
        <div class="event-card__heart mar__t--1" @click="toggleLike">
            ...
            />
            <img v-else class="heart__icon" src="@/assets/heart-empty.svg" />
        </div>
        <!-- Event particulars -->
        <div class="event-card__particulars text--center mar__b--4">
            <div
                class="tagline event-card__particulars--type mar__b--1 text--capsule cursor__default"
            >
                {{ event.type }}
            </div>
            <div class="subheading">{{ event.name }}</div>
            <div class="tagline event-card__particulars--organizer mar__b--2">
                {{ event.organizer }}
            </div>
            <div class="body event-card__particulars--bio mar__b--4">
                {{ event.description }}
            </div>
            <div class="event-card__particulars--link">
                {{ event.link ? event.link : 'No Link Provided' }}
            </div>
            <div class="tagline--bold  event-card__particulars--date">
                {{ event.dates }}
            </div>
        </div>
    </div>
