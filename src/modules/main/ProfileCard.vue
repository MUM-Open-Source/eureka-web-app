    ...
    <div class="profile-card pad--2 mar__b--4">
        <div class="profile-card__top mar__b--2">
            ...
            <div class="profile-card__wave cursor__pointer" @click="toggleWave">
                ...
                />
                <img v-else class="wave__icon" src="@/assets/hand-empty.svg" />
            </div>
        </div>
        <!-- Profile image -->
        <RoundImage
            :src="user.image_url"
            alt="User"
            class="mar__b--1 text--center"
        />
        <!-- User particulars -->
        <div class="profile-card__particulars text--center mar__b--2">
            <div class="subheading">{{ user.full_name }}</div>
            <div
                class="tagline profile-card__particulars--background mar__b--1"
            >
                {{ user.background }}
            </div>
            <div class="body profile-card__particulars--bio">
                {{ user.bio }}
            </div>
        </div>
        <div class="mar__t--auto">
            ...
            <div class="profile-card__socials mar__b--1">
                <a
                    v-if="user.social_links.email_id"
                    :href="'mailto:' + user.social_links.email_id"
                >
                    <img
                        class="profile-card__socials--icon"
                        src="@/assets/email-icon.svg"
                    />
                </a>
                <a
                    v-if="user.social_links.linkedin_url"
                ...
                <a
                    v-if="user.social_links.github_url"
                ...
                <a
                    v-if="user.social_links.website_url"
