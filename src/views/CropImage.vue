<template>
    <div class="cropper__wrapper">
        <cropper
            ...
            transitions="false"
        />
        <div v-if="img.length === 0" class="subheading mar__t--2">
            Loading..
        </div>
...
export default {
    ...
    setup() {
        const img = computed(() =>
            store.state.upload_image.url ? store.state.upload_image.url : ''
        );
        ...
        const cropAndUpload = () => {
            if (
                croppedCoordinates.value.width ==
                croppedCoordinates.value.height
            ) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Sorry!',
                    text: 'Your picture is not within supported size format.',
                });
