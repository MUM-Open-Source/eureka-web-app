    ...
    mutations: {
        ...
        SIGNUP_USER(state, signUpUser: Pick<User, 'first_name' | 'last_name' | 'roles'>) {
            ...
            const checksEmailMatchesDomains = (arrayOfAcceptableDomainNames: string[], userEmail: string): boolean =>
                arrayOfAcceptableDomainNames.map(domain_name => userEmail.toLowerCase().endsWith(domain_name)).includes(true);
            ...
            const evaluatesUserMail = (userEmail: string, arrayOfAcceptableDomainNames: string[], onAccepted: Function, onRejected: Function): void =>
                checksEmailMatchesDomains(arrayOfAcceptableDomainNames, userEmail) ? onAccepted() : onRejected();
            ...
            firebase
                ...
                .then(result => {
                    ...
                    if (result.additionalUserInfo!.isNewUser) {
                        ...
                        else if (signUpUser && signUpUser.roles.includes('mentor')) whenAccept(user);
            ...
            auth.sendPasswordResetEmail(emailId)
                .then(() => {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Email sent',
                        text: 'Please reset your password with the link sent to your inbox.'
                    });
                    router.push('Login');
                })
                .catch(error => {
        ...
        UPLOAD_USER_IMAGE(state, user) {
            ...
            task.then(snapshot => snapshot.ref.getDownloadURL()).then(url => (state.new_img_url = url));
        },
        ...
        GET_USER_NOTIFICATIONS(state) {
            db.collection('notifications')
                ...
                .onSnapshot(querySnapshot => {
                    querySnapshot.forEach(doc => {
                        if (state.notifications.some(notification => notification.id === doc.id)) {
                            const objIndex = state.notifications.findIndex(notification => notification.id == doc.id);
